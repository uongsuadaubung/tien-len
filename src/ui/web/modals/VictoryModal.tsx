  ...
  return (
    ...
    >
      <div className="space-y-3">
        ...
        {/* CHI TIẾT ĐIỂM XẾP HẠNG (ELO BREAKDOWN) */}
        {!isCampaign && lastEloBreakdown && lastEloBreakdown.items.length > 0 && (
          <Card variant="card" className="p-2.5 sm:p-3 space-y-1.5 bg-gradient-to-r from-slate-900/90 via-slate-850/70 to-slate-900/90 border-slate-700/60 shadow-inner">
            <div className="flex items-center justify-between border-b border-white/10 pb-1.5 mb-1">
              <span className="text-[11px] font-bold text-[var(--color-gold)] uppercase tracking-wider flex items-center gap-1.5">
                ⚡ {t('victory.eloBreakdownTitle')}
              </span>
              <span className={`text-xs font-mono font-bold px-1.5 py-0.5 rounded ${eloDelta >= 0 ? 'bg-emerald-950/70 text-emerald-400 border border-emerald-500/30' : 'bg-rose-950/70 text-rose-400 border border-rose-500/30'}`}>
