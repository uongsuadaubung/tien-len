      ...
      footer={
        <div className="w-full flex items-center justify-between gap-2">
          <div className="text-xs text-[var(--text-muted)] truncate">
            {t('quests.progress', { 
              current: tab === 'DAILY' ? completedDailyCount : completedAchCount, 
              target: tab === 'DAILY' ? 5 : profile.achievements.length 
            })}
          </div>
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h4 className="font-bold text-xs sm:text-sm text-[var(--text-primary)] truncate">{quest.title}</h4>
                        <p className="text-[11px] text-[var(--text-muted)] mt-0.5 truncate">{quest.description}</p>
                        <div className="flex items-center gap-2 mt-1.5">
                          <div className="w-24 sm:w-40 h-1.5 bg-[var(--bg-canvas)] rounded-full overflow-hidden border border-[var(--border-container)]">
                            <div
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-2">
                      {quest.isClaimed ? (
                        <Badge variant="dark" size="sm" icon={<CheckCircle className="w-3.5 h-3.5 text-emerald-400" />}>
                          {t('quests.claimed')}
                        </Badge>
                      ) : quest.isCompleted ? (
                        <Button
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <h4 className="font-bold text-xs sm:text-sm text-[var(--text-primary)] truncate">{ach.title}</h4>
                          <span className="text-[9px] px-1.5 py-0.5 rounded bg-[var(--bg-container)] border border-[var(--border-container)] text-[var(--text-muted)] uppercase font-mono shrink-0">
                            {ach.category}
                          </span>
                        </div>
                        <p className="text-[11px] text-[var(--text-muted)] mt-0.5 truncate">{ach.description}</p>
                        <div className="flex items-center gap-2 mt-1.5">
                          <div className="w-24 sm:w-40 h-1.5 bg-[var(--bg-canvas)] rounded-full overflow-hidden border border-[var(--border-container)]">
                            <div
                              className="h-full bg-[var(--color-gold)] rounded-full transition-all duration-300"
