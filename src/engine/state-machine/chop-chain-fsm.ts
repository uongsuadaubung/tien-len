...
/**
 * Hàm thuần túy đánh giá và chuyển đổi trạng thái chặt trong vòng đấu (Functional State Transition)
 */
export function evaluateChopTransition(
  params: EvaluateChopParams,
  currentState: RoundChopState = { type: 'NO_CHOP' }
): ChopTransitionResult {
  const {
    isChopMove,
    chopperId,
    leadingMove,
...
export class ChopChainStateMachine {
  ...
  public evaluateMove(params: EvaluateChopParams): ChopTransitionResult {
    const result = evaluateChopTransition(params, this.currentState);
